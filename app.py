...
if colonna == "1":
    ...
    prompt_colonna = prompt_base + (
        ...
        "CONOSCENZA DI RIFERIMENTO E GUIDA PEDAGOGICA PER LA SEZIONE 2:\n"
        "1. CONTESTO FILOSOFICO E METODO: Zenone di Elea (scuola di Parmenide) utilizza la DIMOSTRAZIONE PER ASSURDO (reductio ad absurdum) "
        "per mostrare che considerare reali il movimento o la molteplicità conduce a contraddizioni logiche.\n"
        "2. I PARADOSSI DEL MOVIMENTO E SUPERTASK: Analizza Achille e la Tartaruga e la Dicotomia (1/2, 1/4, 1/8...). "
        "Fai emergere il concetto di SUPERTASK: l'ostacolo logico di dover completare un numero infinito di operazioni in un tempo finito.\n"
        "3. LA RISPOSTA DI ARISTOTELE: Introduce la distinzione tra INFINITO POTENZIALE (divisibilità indefinita in potenza) "
        "e INFINITO ATTUALE (divisioni reali già compiute). Mostra come Aristotele evidenzi che anche il tempo si suddivide proporzionalmente allo spazio.\n"
        "4. SOLUZIONE MATEMATICA FORMALE (Cauchy, XIX sec.): Mostra come la matematica moderna risolva il paradosso tramite il LIMITE e le SERIE CONVERGENTI. "
        "La somma di infiniti intervalli decrescenti forma una serie geometrica (es. 100 * sum_{k=0}^{infinity} (1/10)^k = 1000/9 = 111.1m; oppure 1 + 1/2 + 1/4 + 1/8... = 2). "
        "L'errore di Zenone sta nel confondere un numero infinito di suddivisioni descrittive con una durata infinita reale.\n"
        "5. DOMANDA APERTA SULLA FISICA REALE: Guida gli studenti a chiedersi se la matematica del continuo (R) descriva davvero il mondo fisico "
        "o se lo spazio-tempo reale sia DISCRETO (struttura a mattoncini/quantistica a scala di Planck).\n"
        "REGOLE MAIEUTICHE: Non svelare mai le formule pronte prima che lo studente avverta il problema concettuale. "
        "Incalzalo chiedendogli se sommare infiniti pezzetti sempre più piccoli (1/2, 1/4, 1/8...) debba per forza dare un totale infinito "
        "o se non si stia avvicinando a un 'soffitto' (limite) ben preciso."
    )
elif colonna == "3":
    prompt_colonna = prompt_base + (
